...
bool ReportHandler::setTimeRange(const QSqlDatabase& configDb, const QSqlDatabase& dataDb, const QJsonObject &recvObj, QString &response)
{
    QSqlQuery testQuery(dataDb);
    int queueId = 0;
    QString strSql = QString("select * from queue where current=1");
    ...
    if (!testQuery.next()) {
        QJsonObject jsonObj;
        jsonObj["__channel"] = channel_ + "-set-time-range";
        jsonObj["queue_id"] = 0;
        QJsonDocument jsonDoc(jsonObj);
        response = jsonDoc.toJson();
        return true;
    ...
    qint64 startTime = recvObj["start_time"].toVariant().toLongLong();
    qint64 endTime = recvObj["end_time"].toVariant().toLongLong();

    //仅保留 sampleTimeUs 在 [startTime, endTime] 范围内的数据
    ...
    updateQuery.bindValue(":id", queueId);
    if (!updateQuery.exec()) {
    ...
    jsonObj["__channel"] = channel_ + "-set-time-range";
    jsonObj["queue_id"] = queueId;
    jsonObj["total"] = static_cast<int>(newTotal);
...
bool ReportHandler::handleWsMsg(QJsonObject &recvObj, QString &response)
{
	...
	else if (type == "set-time-range")
	{
		return setTimeRange(configDb, testDataDb, recvObj, response);
	}
